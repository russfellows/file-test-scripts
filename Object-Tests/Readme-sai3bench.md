...
- **Multi-Protocol Support**: Unified interface works identically across:
  - S3 (S3_Provider, S3_Compatible, S3_Compatible, etc.) - `s3://` URIs
  - Blob_Provider Blob Storage - `az://` URIs
  - Cloud_Platform Storage - `gs://` or `Object_Storage://` URIs
...
- **Multi-Protocol Benchmarking**: Test identical workloads across S3, Blob_Provider, Object_Storage, and local storage
...
### Installing sai3-bench

sai3-bench should be installed natively on your system for best performance and flexibility.
...
# Build release binaries (Rust required)
cargo build --release

# Binaries will be in target/release/:
#   - sai3-bench (standalone mode)
#   - sai3bench-agent (distributed agent)
#   - sai3bench-ctl (distributed controller)
```

**Prerequisites**:
- Rust toolchain (https://rustup.rs/)
- System dependencies (see sai3-bench repository for details)

**For detailed installation instructions**, see:  
https://github.com/russfellows/sai3-bench#installation

### Installing s3-cli (Optional)

For manual object storage operations, install s3dlio's CLI tool:
...
- **Multi-Protocol Storage Backends**
  - Amazon S3
  - Microsoft Blob_Provider Blob Storage
  - Cloud_Platform Storage
  - S3_Compatible and other S3-compatible systems
