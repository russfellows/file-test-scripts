...
⚠️ **IMPORTANT**: This container must be built locally.
...
**For detailed instructions**, see: [../../dockerfiles/io500/](../../dockerfiles/io500/)
...
- **io500-mpi-coordinate-gemini-Cloud_Platform.sh** - Distributed test coordination script for Cloud_Platform
- **drop-cache-Cloud_Platform.sh** - Cache clearing utility for consistent benchmarking
...
- Operating System: **Ubuntu 22.04 LTS**
  - ⚠️ Note: Ubuntu 24.04 is not compatible with certain parallel mount types as of current release
- Shared file system: **Parallel File System** (or compatible HPC parallel file system)

### Pre-Flight Checklist

Before running tests, ensure:
- [ ] Parallel File System prerequisites installed on all VMs
- [ ] Parallel File System file system mounted at `/mnt/storage` (or configured path) on all VMs
...
**io500-mpi-coordinate-gemini-Cloud_Platform.sh** - For Cloud_Platform infrastructure
- Configures SSH access to Generic Compute Engine instances
- Sets up MPI environment for distributed Cloud_Platform execution
- Edit to include your Cloud_Platform username, SSH key path, instance names
- Configure shared file system mount points
- Output directory for results

**drop-cache-Cloud_Platform.sh** - Cache clearing utility
...
- `io500-mpi-coordinate-gemini-Cloud_Platform.sh` coordination script
...
```bash
vim io500-mpi-coordinate-gemini-Cloud_Platform.sh
```
...
```bash
bash io500-mpi-coordinate-gemini-Cloud_Platform.sh
```
...
bash drop-cache-Cloud_Platform.sh
```

**What `drop-cache-Cloud_Platform.sh` does**:
...
4. **Use same configuration and coordination approach**:
   ...
   - Ensure all VMs have shared file system access (Parallel File System, NFS, etc.)
...
- **IOR Component** (used by IO500 for I/O benchmarking):
  ...
  - IOR Manual Pages available in container:
    ```bash
    docker run -it io500:latest man ior
    ```
...
- Container build instructions: [../../dockerfiles/io500/](../../dockerfiles/io500/)
