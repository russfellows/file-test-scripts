...
for H in "${HOSTS[@]}"; do
  ...
  ssh "${VM_USER}@${H}" "sudo docker rm -f io500 >/dev/null 2>&1 || true;
    sudo docker run -d --name io500 --restart unless-stopped --network host --user 0:0 -v /mnt/storage:/mnt/storage -v /mnt/data:/mnt/data -v ~/root/.ssh:/root/.ssh -v /tmp:/tmp ${HOSTKEY_MOUNT} ${IMAGE} bash -lc 'exec sleep infinity'"
...
mpirun --hostfile ${HOST_FILE} --mca plm_rsh_agent 'ssh -p 2222 -o StrictHostKeyChecking=no -o IdentitiesOnly=yes' \\
       ...
       /opt/io500/io500 /mnt/storage/scripts/IOR-MDtest-full.ini
EOF
chmod +x "${RUN_SCRIPT}"
