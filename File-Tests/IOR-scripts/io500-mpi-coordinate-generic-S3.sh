...
###############################################
LAUNCHER_KEY="${HOME}/.ssh/S3_Provider-Generic-keypair.pem"
...
# Generate the Public Key string from the Private PEM (S3_Provider doesn't give you a .pub file)
...
for H in "${HOSTS[@]}"; do
  ...
  ssh "${SSH_OPTS[@]}" "${VM_USER}@${H}" "sudo docker rm -f ${CONTAINER_NAME} >/dev/null 2>&1 || true;
    ...
    -v /mnt/storage:/mnt/storage -v /mnt/data:/mnt/data \
...
# --- GENERATE HOSTFILE ---
HOST_FILE="/mnt/storage/Scripts/ior-hosts.txt"
...
RUN_SCRIPT="/mnt/storage/Scripts/start_io500.sh"
cat > "${RUN_SCRIPT}" <<EOF
#!/bin/bash
...
mpirun --hostfile ${HOST_FILE} --mca plm_rsh_agent 'ssh -p 2222 -o StrictHostKeyChecking=no -o IdentitiesOnly=yes' \\
       ...
       /opt/io500/io500 /mnt/storage/Scripts/IOR-MDtest.ini
EOF
chmod +x "${RUN_SCRIPT}"
...
echo
echo "  nohup sudo docker exec ${CONTAINER_NAME} ${RUN_SCRIPT} > /mnt/storage/Scripts/io500_run.log 2>&1 &"
echo
echo "Then you can disconnect. Tail the log to watch progress:"
echo "  tail -f /mnt/storage/Scripts/io500_run.log"
echo "----------------------------------------------------------------"
echo
